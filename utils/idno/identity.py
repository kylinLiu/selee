...
# 导入常量并重命名
import utils.idno.constant as const
...
class IdNumber(str):
    ...
    @classmethod
    def generate_id_sz_ba(cls, sex=1, year='1999'):
        """随机生成身份证号，sex = 0表示女性，sex = 1表示男性"""

        # 随机生成一个区域码(6位数)
        id_number = '440306'
        # 限定出生日期范围(8位数)
        start, end = datetime.strptime(f"{year}-01-01", "%Y-%m-%d"), datetime.strptime(f"{year}-12-30", "%Y-%m-%d")
        # start, end = datetime.strptime("1960-01-01", "%Y-%m-%d"), datetime.strptime("2000-12-30", "%Y-%m-%d")
        birth_date = datetime.strftime(start + timedelta(random.randint(0, (end - start).days + 1)), "%Y-%m-%d")
        birth_days = birth_date.replace('-', '')
        ...
        # 校验码(1位数)
        return id_number + str(cls(id_number).get_check_digit()), birth_date

    @classmethod
    def generate_id_sz_ns(cls, sex=0, year='2000'):
        """随机生成身份证号，sex = 0表示女性，sex = 1表示男性"""
    ...
    @classmethod
    def csrq(cls, idcard):
        return f"{idcard[-12:-8]}-{idcard[-8:-6]}-{idcard[-6:-4]}"

    @classmethod
    def generate_id(cls, sex=0, year='2020'):
        """随机生成身份证号，sex = 0表示女性，sex = 1表示男性"""
...
if __name__ == '__main__':
    ...
    # print(IdNumber.generate_id(random_sex, '2022'))  # 随机生成身份证号
    ...
    # print(IdNumber.verify_id('220122199907120015'))  # 检验身份证是否正确:False
    ids = [
        # '51022519651224358X',
        # '440306201912210180',
        # '500381198911074406',
    ...
    for id in ids:
        print(id, IdNumber.verify_id(id))
